...
class graspRGB(imdb):
    def __init__(self, image_set, devkit_path):
        ...
        self._data_path = os.path.join(self._devkit_path, 'grasp_data')
    ...
    def _load_graspRGB_annotation(self, index):
        ...
        """
        #index = index.split('/')[-1]
        #filename = self._data_path + '/Annotations/'+ index[:-4] + '.txt'
        filename = os.path.join(self._data_path, 'Annotations', index + '.txt')
        # print('data_path: {}'.format(self._data_path))
        # print('index: {}'.format(index[:-4]))
